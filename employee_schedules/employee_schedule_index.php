...
<body>
<div class="container">
    <h1>Employee Schedule Assignment</h1>

    <div class="card">
        <h2>Add New Schedule Assignment</h2>
        <div class="form-row">
            ...
            <div class="form-group">
                <label for="new_schedule_id">Work Schedule</label>
                <select id="new_schedule_id" required>
                    <option value="">Select Schedule</option>
                    <?php
                    $result = $conn->query("SELECT schedule_id, name FROM work_schedules");
                    while ($row = $result->fetch_assoc()):
        ...
        <div class="form-group">
            <label for="new_assigned_by">Assigned By (Optional)</label>
        ...
        <button onclick="assignSchedule()" class="success">➕ Assign Schedule</button>
    </div>

    <div class="card">
        <h2>Existing Employee Schedules</h2>
        <table>
            <thead>
                <tr>
                    <th>ID</th>
            ...
            <tbody>
                <?php
                $sql = "SELECT e.id, e.employee_id, e.schedule_id, e.effective_from, e.effective_to,
                               CONCAT(emp.first_name, ' ', emp.last_name) AS employee,
                               s.name AS schedule_name,
                               u.username AS assigned_by
                        FROM employee_schedules e
                        JOIN employees emp ON e.employee_id = emp.employee_id
                        JOIN work_schedules s ON e.schedule_id = s.schedule_id
                ...
                <tr id="row_<?= $row['id'] ?>">
                    ...
                    <td><?= htmlspecialchars($row['schedule_name']) ?></td>
                    <td>
                        <input type="date" id="from_<?= $row['id'] ?>" value="<?= $row['effective_from'] ?>">
                        - 
                        <input type="date" id="to_<?= $row['id'] ?>" value="<?= $row['effective_to'] ?? '' ?>">
                    </td>
                    <td><?= htmlspecialchars($row['assigned_by'] ?? '') ?></td>
                    <td>
                        <div class="action-buttons">
                            <button onclick="updateScheduleAssignment(<?= $row['id'] ?>)" class="success">💾 Save</button>
                            <button onclick="deleteScheduleAssignment(<?= $row['id'] ?>)" class="danger">🗑️ Delete</button>
                        </div>
                    </td>
                </tr>
                <?php endwhile; ?>
...
<script>
    ...
    function updateScheduleAssignment(id) {
        const from = document.getElementById(`from_${id}`).value;
        const to = document.getElementById(`to_${id}`).value || null;

        if (!from) {
            showNotification("❌ Effective From is required", false);
            return;
        }

        fetch('update_employee_schedule.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            body: `id=${id}&effective_from=${from}&effective_to=${to}`
        })
        .then(response => {
    ...
    function assignSchedule() {
        ...
        const scheduleId = document.getElementById('new_schedule_id').value;
        ...
        fetch('add_employee_schedule.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            body: `employee_id=${employeeId}&schedule_id=${scheduleId}&effective_from=${effectiveFrom}&effective_to=${effectiveTo}&assigned_by=${assignedBy}`
        })
        .then(response => {
            if (!response.ok) throw new Error('Network error');
            return response.text();
    ...
    function deleteScheduleAssignment(id) {
        if (!confirm("Are you sure you want to delete this schedule assignment?")) return;

        fetch('delete_employee_schedule.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            body: `id=${id}`
        })
